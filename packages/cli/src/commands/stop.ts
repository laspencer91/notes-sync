...
export async function stopCommand() {
  ...
  try {
    ...
    try {
      ...
      console.log("⚠️ Could not connect to service API, trying alternative methods");
    }
    ...
    if (process.platform === "darwin") {
      ...
      if (fs.existsSync(plistPath)) {
        console.log("🔄 Temporarily unloading LaunchAgent to prevent auto-restart");
    ...
    if (process.platform === "darwin" || process.platform === "linux") {
      ...
      if (findCmd.status === 0) {
        const processes = findCmd.stdout
          .split("\n")
          .filter(line => line.includes("notes-sync-service") && !line.includes("grep"));
        
        if (processes.length > 0) {
          console.log(`Found ${processes.length} service processes still running`);
